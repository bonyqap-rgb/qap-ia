...
import { Share2, MessageSquarePlus, Filter, Clock, User, FileText, CheckCircle2 } from "lucide-react";
...
function SharedPage() {
  ...
  const items = MOCK_SHARED.filter(item => 
    ...
    <Container size="wide" className="py-6 sm:py-8 space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="grid gap-6">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Badge tone="neutral" className="cursor-pointer hover:bg-muted/80">Pareceres</Badge>
          <Badge tone="neutral" className="cursor-pointer hover:bg-muted/80">Consultas</Badge>
          ...
          <Badge tone="neutral" className="cursor-pointer hover:bg-muted/80">Recentemente Adicionados</Badge>
        </div>

        <Panel 
          className="overflow-hidden p-1"
          title="Biblioteca de Compartilhamentos" 
          description={items.length > 0 ? `${items.length} itens encontrados` : "Nenhum compartilhamento disponível"}
        >
          {items.length > 0 ? (
            ...
            <EmptyState
              ...
              description={search ? `Não encontramos nada para "${search}"` : "As consultas compartilhadas com você aparecerão nesta área."}
              action={
                search ? (
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <Card interactive padding="sm" className="flex flex-col gap-3">
          ...
          <div>
            ...
            <p className="text-caption text-muted-foreground">Compartilhado por Maj PM Oliveira para toda a corporação.</p>
          </div>
          <div className="pt-2 flex items-center justify-between border-t border-border/40">
            <span className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Manual</span>
