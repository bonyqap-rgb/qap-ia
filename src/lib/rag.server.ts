...
const LITERAL_INTENT =
  /\b(transcreva|transcri(?:ção|cao)|texto\s+(?:integral|literal|do\s+artigo)|conte[uú]do\s+d[oa]\s+artigo|o\s+que\s+diz|qual\s+(?:é|e)\s+o\s+(?:texto|conte[uú]do)|reproduz|cite\s+o\s+artigo|copie)\b/i;
